...
describe('version-item.ts', () => {
  it('should render a version item with label, value, and GitHub link from entity', async () => {
    ...
    expect(labelEl?.textContent).to.equal('AdGuard Home');
    ...
    expect(valueEl?.textContent).to.contain('v0.107.79');
  });
  ...
  it('should add update-available class and show latest version when update is available', async () => {
    ...
    const entity: EntityInformation = {
      ...
      attributes: {
        friendly_name: 'AdGuard Home update',
        installed_version: 'v0.107.79',
        latest_version: 'v0.107.80',
        release_url:
          'https://github.com/AdguardTeam/AdGuardHome/releases/tag/v0.107.80',
      },
    };
    ...
    expect(versionTextEl?.textContent).to.include('v0.107.79');
    expect(versionTextEl?.textContent).to.include('v0.107.80');
    ...
    expect(latestVersionEl?.textContent).to.equal('v0.107.80');
  ...
  it('should not add update-available class when no update is available', async () => {
    ...
    const versionTextEl = el.querySelector('.version-text');
    expect(versionTextEl).to.exist;
    expect(versionTextEl?.textContent).to.contain('v0.107.79');
  });
});
