...
It is usually not a module-resolution problem. The path aliases (`@cards/*`, `@delegates/*`, and similar aliases) are configured in `tsconfig.json` and registered at runtime by the test setup. Home Assistant imports use `@homeassistant-extras/hass/...`.
